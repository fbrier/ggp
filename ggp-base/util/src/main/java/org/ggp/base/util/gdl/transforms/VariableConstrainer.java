...
public class VariableConstrainer {
    ...
    private static ListMultimap<SentenceForm, Ambiguity> getAmbiguitiesByOriginalForm(
        ...
        for (Ambiguity ambiguity : result.values()) {
            ...
            if (! allForms.contains(ambiguity.getOriginal()))
                throw new RuntimeException( "allForms.contains(ambiguity.getOriginal())" );
            // Assert.assertTrue(allForms.contains(ambiguity.getReplacement()));
            if (! allForms.contains(ambiguity.getReplacement()))
                throw new RuntimeException( "allForms.contains(ambiguity.getReplacement())" );
        }
